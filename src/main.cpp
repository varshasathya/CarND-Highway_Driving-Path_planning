...
int main() {
  ...
  h.onMessage([&lane, &ref_velocity, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < sensor_fusion.size(); i++) {
            ...
            speed_check = sqrt(vx*vx+vy*vy);
            double check_car_s = sensor_fusion[i][5];
            // predict where the vechicle will be in future
            check_car_s += ((double)prev_size*.02*speed_check); 
          ...
          // redefine the previous point as reference point
          else {
          ...
          // Start with all of the previous path points for smooth transition
